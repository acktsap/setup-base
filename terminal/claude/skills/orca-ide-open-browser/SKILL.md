...
2. Resolve the target worktree explicitly: run `/usr/local/bin/orca worktree current --json` and read `result.worktree.id` and `result.worktree.branch`. Build the selector as `id:<result.worktree.id>` when `id` is present; otherwise use `branch:<result.worktree.branch>`. Quote the whole `--worktree` value because ids contain `::` and `/`. Use this same selector verbatim in every step below. Only override this default (e.g. `path:<repo-root>` for the main workspace) when the user says they're viewing a different window.
